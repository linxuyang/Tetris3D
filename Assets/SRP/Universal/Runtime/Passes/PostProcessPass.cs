...
namespace UnityEngine.Rendering.Universal.Internal
{
    public class PostProcessPass : ScriptableRenderPass
    {
        ...
        MaterialLibrary m_Materials;    //后处理用到的全部材质
        PostProcessData m_Data;         //后处理的设置资源
        ...
        {
            float ratio = m_Bloom.anamorphicRatio.value;
            float widthRatio = Mathf.Max(-ratio, 0);
            float heightRatio = Mathf.Max(ratio, 0);
            int tw = Mathf.FloorToInt(m_Descriptor.width / (4 - widthRatio));
            int th = Mathf.FloorToInt(m_Descriptor.height / (4 - heightRatio));

            // Determine the iteration count
            int maxSize = Mathf.Max(tw, th);
            float logs = Mathf.Log(maxSize, 2f) + Mathf.Min(m_Bloom.diffusion.value, 10f) - 10f;
            int logs_i = Mathf.FloorToInt(logs);
            int iterations = Mathf.Clamp(logs_i, 1, m_Bloom.sampleTimes.value);
            float sampleScale = 0.5f + logs - logs_i;

            // Pre-filtering parameters
            float clamp = m_Bloom.clamp.value;
            float threshold = Mathf.GammaToLinearSpace(m_Bloom.threshold.value);
            float knee = threshold * m_Bloom.softKnee.value + 1e-5f;
            // float thresholdKnee = threshold * 0.5f; // Hardcoded soft knee

            // Material setup
            // float scatter = Mathf.Lerp(0.05f, 0.95f, m_Bloom.scatter.value);
            var bloomMaterial = m_Materials.bloom;
            bloomMaterial.SetFloat(ShaderConstants._Clamp, clamp);
            bloomMaterial.SetVector(ShaderConstants._Threshold, new Vector4(threshold, threshold - knee, knee * 2f, 0.25f / knee));
            bloomMaterial.SetFloat(ShaderConstants._SampleScale, sampleScale);
            CoreUtils.SetKeyword(bloomMaterial, "ANTI_FLICKER", m_Bloom.fastMode.value && m_Bloom.antiFlicker.value);
            
            int qualityOffset = m_Bloom.fastMode.value ? 1 : 0;
            // Prefilter
            ...
            Blit(cmd, source, ShaderConstants._BloomMipDown[0], bloomMaterial, 0 + qualityOffset);
            
            // Downsample - gaussian pyramid
            int lastDown = ShaderConstants._BloomMipDown[0];
            int pass = 2 + qualityOffset;
            for (int i = 1; i < iterations; i++)
            {
                ...
                Blit(cmd, lastDown, mipDown, bloomMaterial, pass);
                lastDown = mipDown;
            }

            int lastUp = ShaderConstants._BloomMipDown[iterations - 1];
            pass = 4 + qualityOffset;
            for (int i = iterations - 2; i >= 0; i--)
            {
                int mipDown = ShaderConstants._BloomMipDown[i];
                int mipUp = ShaderConstants._BloomMipUp[i];
                cmd.SetGlobalTexture(ShaderConstants._SourceTexLowMip, mipDown);
                Blit(cmd, lastUp, BlitDstDiscardContent(cmd, mipUp), bloomMaterial, pass);
                lastUp = mipUp;
            }
            ...
            // Setup bloom on uber
            float intensity = Mathf.Exp(m_Bloom.intensity.value / 10f * math.LN2) - 1f;
            
            uberMaterial.EnableKeyword(ShaderKeywordStrings.Bloom);
            
            uberMaterial.SetFloat(ShaderConstants._SampleScale, sampleScale);
