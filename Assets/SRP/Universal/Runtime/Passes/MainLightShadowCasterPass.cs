...
namespace UnityEngine.Rendering.Universal.Internal
{
    ...
    public class MainLightShadowCasterPass : ScriptableRenderPass
    {
        private static class MainLightShadowConstantBuffer
        {
            ...
            public static int _CustomShadowCenter;
        ...
        public static Vector4 customShadowParams; // xyz:自定义阴影生效区域中心点; w:自定义阴影生效区域半径
        ...
        public MainLightShadowCasterPass(RenderPassEvent evt)
        {
            ...
            // 自定义阴影的中心(一般是玩家角色的世界坐标)
            MainLightShadowConstantBuffer._CustomShadowCenter = Shader.PropertyToID("_CustomShadowCenter");
            ...
            using (new ProfilingScope(cmd, ProfilingSampler.Get(URPProfileId.MainLightShadow)))
            {
                ...
                for (int cascadeIndex = 0; cascadeIndex < m_ShadowCasterCascadesCount; ++cascadeIndex)
                {
                    ...
                    CoreUtils.SetKeyword(cmd, ShaderKeywordStrings.CastingPunctualLightShadow, false);
                        //设置相机矩阵和投影矩阵，相当于把相机放在光源处，所以在顶点着色器中可以把顶点转到裁剪空间
        ...
        void SetupMainLightShadowReceiverConstants(CommandBuffer cmd, VisibleLight shadowLight, bool supportsSoftShadows)
        {
            ...
            float shadowDistance = m_MaxShadowDistanceSq;
            // 替换Shadows.hlsl中GetShadowFade的参数
            if (ShadowUtils.useCustomMatrix)
            {
                shadowDistance = customShadowParams.w;
                cmd.SetGlobalVector(MainLightShadowConstantBuffer._CustomShadowCenter, new Vector4(customShadowParams.x, customShadowParams.y, customShadowParams.z, 1));
            }
            else
            {
                cmd.SetGlobalVector(MainLightShadowConstantBuffer._CustomShadowCenter, Vector4.zero);
            }
            
            ShadowUtils.GetScaleAndBiasForLinearDistanceFade(shadowDistance, m_CascadeBorder, out float shadowFadeScale, out float shadowFadeBias);
