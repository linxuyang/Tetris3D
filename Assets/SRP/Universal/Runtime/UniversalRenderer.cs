...
namespace UnityEngine.Rendering.Universal
{
    ...
    {
        ///在一个pass中渲染全部物件，硬性限制一个物件上的灯光数量
        Forward,
        ///先在g-buffer pass中渲染全部物件，然后在一个分离的pass中应用所有光照
        Deferred
    };
    ...
    public sealed class UniversalRenderer : ScriptableRenderer
    {
        ...
        // 渲染路径，来自UniversalRendererData的设置，默认为Forward
        internal RenderingMode renderingMode => m_RenderingMode;

        // 实际渲染路径，有可能不同，(如: wireframe rendering, harware not capable of deferred rendering).
        ...
        //以下除了m_Active的，都是在构造函数创建
        internal RenderTargetBufferSystem m_ColorBufferSystem;  

        RenderTargetHandle m_ActiveCameraColorAttachment;   //当前使用的Color
        RenderTargetHandle m_ColorFrontBuffer;  
        RenderTargetHandle m_ActiveCameraDepthAttachment;   //当前使用的Depth
        RenderTargetHandle m_CameraDepthAttachment; 
        RenderTargetHandle m_DepthTexture;  //DepthPrepass和CopyDepthPass的目标
        RenderTargetHandle m_NormalsTexture;   
        RenderTargetHandle m_OpaqueColor;   //CopyColorPass目标
        // For tiled-deferred shading.
        RenderTargetHandle m_DepthInfoTexture;  
        RenderTargetHandle m_TileDepthInfoTexture; 

        ForwardLights m_ForwardLights;  //前向灯光数据
        DeferredLights m_DeferredLights;    //延迟灯光数据
        RenderingMode m_RenderingMode;  //渲染路径
        DepthPrimingMode m_DepthPrimingMode; //深度启动模式
        CopyDepthMode m_CopyDepthMode;  //深度拷贝模式，AfterOpaques或ForcePrepass
        ...
        public override void Setup(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            //有rendererFeatures且中间纹理模式为Always时需要创建colorTexture
            ...
            AddRenderPasses(ref renderingData); //主要将features的pass添加进来
            isCameraColorTargetValid = false;
            // 统计所有pass的输入概括，是否各种图，如需要深度图什么的
            RenderPassInputSummary renderPassInputs = GetRenderPassInputs(ref renderingData);

            // 相机勾选了后处理，并且PostProcessData不为空
            ...
            // 是否需要深度图，相机面板勾选，m_DepthPrimingMode来自UniversalRendererData面板设置，默认关闭
            ...
            else
            {
                ...
                // 有CopyColorPass或CopyDepthPass时用Store
            ...
            if (lastCameraInTheStack)
            {
                ...
                // 如果不是渲染到rt，没有后处理时，需要加FinalBlitPass，有后处理会自己Blit到camera target
        ...
        public List<ScriptableRendererFeature> GetRenderFeatures() 
        {
