Shader "Hidden/Universal Render Pipeline/UberPost"
{
    HLSLINCLUDE
        ...
        half4 Frag(Varyings input) : SV_Target
        {
            ...
            #if _BLOOM
                half3 bloom = UpsampleBox(TEXTURE2D_ARGS(_Bloom_Texture, sampler_Bloom_Texture), uvDistorted, _Bloom_Texture_TexelSize.xy, _SampleScale).rgb;
                #if UNITY_COLORSPACE_GAMMA
                bloom.xyz *= bloom.xyz; // γ to linear
                #endif
                half base_br = Max3(color.r, color.g, color.b);
                base_br = saturate(lerp(1, pow(base_br, 10), base_br));
                bloom *= base_br;
